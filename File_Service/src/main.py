...
def serve(dependency_manager_instance: DependencyManager):
    ...
    file_service_pb2_grpc.add_FileEmbeddingSubServiceServicer_to_server(
        FileEmbeddingSubService(dependency_manager_instance),
        server
    )
...
if __name__ == '__main__':
    ...
    dependency_manager.add_dependency('orchestration_service_client',
                                      OrchestrationServiceClient("orchestration-service:55000"))
    dependency_manager.add_dependency('minio_client', MinioClient(endpoint="minio:9000", access_key="minioadmin",
                                                                  secret_key="minioadmin"))
    dependency_manager.add_dependency('file_repository',
                                      FileRepository("postgresql://postgres:postgres@postgres/computer_science_ia"))
    dependency_manager.add_dependency('weaviate_client', WeaviateClient("weaviate", 8080, 50051))
    dependency_manager.add_dependency('grobid_client', GrobidClient("http://grobid:8070"))
