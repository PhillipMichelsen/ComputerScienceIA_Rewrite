...
def create_job(
    ...
    service_id: str,
    request_id: str,
    initial_job_data: dict,
    dependency_manager: DependencyManager,
):
    manager_redis_client: ManagerRedisClient = dependency_manager.get_dependency(
        "manager_redis_client"
    )
    worker_redis_client: WorkerRedisClient = dependency_manager.get_dependency(
        "worker_redis_client"
    )
    grpc_service_manager: GrpcServiceManager = dependency_manager.get_dependency(
    ...
    dispatch_task(
        manager_redis_client.dequeue_task_chain(job_id), job_id, dependency_manager
    )
...
def add_task_to_job(job_id: str, task_name: str, dependency_manager: DependencyManager):
    ...
    manager_redis_client.set_job_metadata(
        job_id, {"task_chain": task_chain + "," + task_id}
    )
...
def notify_job(task_id: str, notification: dict, dependency_manager: DependencyManager):
    ...
    worker_redis_client.publish_request_notification(
        service_id, request_id, dict(notification)
    )
    logging.debug(
        f"Notification published for request_id: {request_id} to service_id: {service_id}..."
    )
